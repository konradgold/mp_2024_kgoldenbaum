...
if __name__ == "__main__":
    ...
    net = WSAD(config.len_feature, flag = Mode.TRAIN, a_nums = 60, n_nums = 60)
    net = net.cuda()

    normal_train_loader = data.DataLoader(
        UCF_crime(root_dir = config.root_dir, mode = Mode.TRAIN, modal = config.modal, num_segments = 200, len_feature = config.len_feature, is_normal = True),
            batch_size = 64,
