...
from utils.dmu.utils import norm
...
class ADCLS_head(Module):
    def __init__(self, in_dim, out_dim):
        super().__init__()
        self.mlp = nn.Sequential(nn.Linear(in_dim,128), nn.ReLU(), nn.Linear(128,out_dim))
    def forward(self, x):
        return self.mlp(x)

class WSAD(Module):
    def __init__(self, input_size, flag: Mode = Mode.EVAL, a_nums: int = 64, n_nums: int = 64):
        super().__init__()
        self.flag = flag
        self.a_nums = a_nums
        self.n_nums = n_nums
    ...
    def forward(self, x):
        if len(x.size()) == 6:
            b, n, t, c, w, h = x.size()
            x = x.reshape(b * n, t, c, w,h)
        elif len(x.size()) == 5:
            b, t, c, w,h = x.size()
            n = 1
        else:
            if self.flag == Mode.TRAIN or self.flag == Mode.PRETRAIN:
                raise ValueError("Input shape should be [b, n, t, c, w, h] or [b, t, c, w, h] in training mode but got {}".format(x.size()))
            t, c, w, h = x.size()
            b = 1
            n = 1
            x = x.unsqueeze(0)
        ...
        with torch.no_grad():
            x = self.embedding(x)
            if isinstance(self.embedding, DMUEmbedder) and self.flag == Mode.PRETRAIN:
                b *= 2
                assert x.size(0) == b, f"Expected size {b} but got {x.size(0)}"
        x = self.selfatt(x)
...
if __name__ == "__main__":
    m = WSAD(input_size = 1024, flag = Mode.TRAIN, a_nums = 60, n_nums = 60).cuda()
